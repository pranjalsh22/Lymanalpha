    ...
    with st.expander(f"{spec['object']} ({spec['instrument']})",expanded=False):
        ...
        if (ps_fft is not None and ps_lomb is not None ):
            ...
            with st.expander("A) Raw Power Spectra:"):
                ...
                fig_lomb = go.Figure()
                fig_lomb.add_trace(go.Scatter(x=ps_lomb["logk"],y=ps_lomb["logkpk"],mode="lines",name="Lomb-Scargle"))

                fig_lomb.update_layout(title=(f"Lomb-Scargle Power Spectrum (Window={window_size}, Bins=20)"),
                    xaxis_title="log₁₀(k / km⁻¹ s)",
                    yaxis_title="log₁₀(kP(k)/π)")
